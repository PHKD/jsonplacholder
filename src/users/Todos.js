...
class Todos extends React.Component{
  ...
  addData() {
    axios.post('https://jsonplaceholder.typicode.com/todos', this.state.newDataData).then((response) => {
  ...
  updateData() {
    let { userId,title, completed } = this.state.editDataData;

    axios.put('https://jsonplaceholder.typicode.com/todos' + this.state.editDataData.id, {
      userId,title, completed
    }).then((response) => {
      this.getData();

      this.setState({
        editDataModal: false, editDataData: { userId:'',id: '', title: '', completed: '' }
      })
    });
  }
  editData(userId,id, title, completed) {
    this.setState({
      editDataData: {userId,id, title, completed }, editDataModal: ! this.state.editDataModal
    });
  }
  deleteData(id) {
  ...
  searchData(userId) {
    axios.get('https://jsonplaceholder.typicode.com/todos?userId='+this.state.value).then((response) => {
      this.setState({
        posts: response.data
      })
    });
  ...
  render() {
    let posts = this.state.posts.map((post) => {
      return (
        <tr key={post.id}>
          ...
          <td>{post.id}</td>
          <td>{post.title}</td>
          <td>{post.completed.toString()}</td>
          <td>
            <Button color="success" size="sm" className="mr-2" onClick={this.editData.bind(this,post.userId, post.id, post.title, post.completed)}>Edit</Button>
    ...
    return (
      ...
      <Button className="my-3" color="primary" onClick={this.toggleAddDataModal.bind(this)}>Add Todo</Button>

      <Modal isOpen={this.state.newDataModal} toggle={this.toggleAddDataModal.bind(this)}>
        <ModalHeader toggle={this.toggleAddDataModal.bind(this)}>Add a new todo</ModalHeader>
        <ModalBody>
        <FormGroup>
          ...
          <FormGroup>
            ...
            <Input id="completed" value={this.state.newDataData.completed} onChange={(e) => {
              let { newDataData } = this.state;

              newDataData.completed = e.target.value;
        ...
        <ModalFooter>
          <Button color="primary" onClick={this.addData.bind(this)}>Add Todo</Button>{' '}
      ...
      <Modal isOpen={this.state.editDataModal} toggle={this.toggleEditDataModal.bind(this)}>
        <ModalHeader toggle={this.toggleEditDataModal.bind(this)}>Edit a todo</ModalHeader>
        <ModalBody>
        <FormGroup>
            <Label for="userId">UserId</Label>
        ...
        <FormGroup>
          ...
          <FormGroup>
            <Label for="completed">completed</Label>
            <Input id="completed" value={this.state.editDataData.completed} onChange={(e) => {
              let { editDataData } = this.state;

              editDataData.completed = e.target.value;
        ...
        <ModalFooter>
          <Button color="primary" onClick={this.updateData.bind(this)}>Update todo</Button>{' '}
